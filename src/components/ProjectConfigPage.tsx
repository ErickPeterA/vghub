...
const ROLE_OPTIONS: ProjectRoleValue[] = [
  "admin",
  "gp",
  "lider_estrategico",
  "lider_tatico",
...
const inputClass =
  "w-full rounded-lg border border-[#042558]/20 bg-white/50 px-3 py-2.5 text-sm text-[#042558] outline-none transition-all focus:border-[#042558] focus:ring-2 focus:ring-[#042558]/20 placeholder:text-[#042558]/40";
            ...
            <div className="space-y-4">
              {members.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="overflow-hidden rounded-xl border border-[#042558]/10 bg-white/50 transition-all hover:border-[#042558]/30 hover:shadow-md"
                  >
                        ...
                        <div className="space-y-4">
                          ...
                          <ScopePicker
                            ...
                            renderLabel={(a) => {
                              const parent = a.parent_id
                                ? allAreas.find((x) => x.id === a.parent_id)
                                : null;
                          ...
                          {memberScopes.filter((s) => {
                            ...
                            <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700">
                              <span className="h-1.5 w-1.5 rounded-full bg-amber-500" />⚠ Este líder
                              ainda não tem nenhuma área vinculada.
                            </div>
                          )}
                        </div>
                      </div>
      ...
      <div className="mb-2 flex flex-wrap gap-2">
        {selected.map((s) => {
          ...
          return (
            <span
              key={s.id}
              className="inline-flex items-center gap-2 rounded-full border border-[#042558]/20 bg-white/60 px-3 py-1 text-xs font-medium text-[#042558]"
            >
              {label(a)}
              <button
                onClick={() => onRemove(s)}
