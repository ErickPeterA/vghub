  ...
  return (
    <main className="mx-auto max-w-xl px-6 py-10">
      ...
      <form onSubmit={submit} className="space-y-4 rounded-xl border border-border bg-card p-6">
        <Field label="Nome"><input required value={nome} onChange={(e) => setNome(e.target.value)} className={inp} /></Field>
        <Field label="E-mail"><input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className={inp} /></Field>
        <Field label="Senha"><input type="text" required value={password} onChange={(e) => setPassword(e.target.value)} className={inp} /></Field>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={makeAdmin} onChange={(e) => setMakeAdmin(e.target.checked)} /> Conceder permissão de Administrador</label>
        <div className="flex gap-2 pt-2">
          <button type="button" onClick={() => navigate({ to: "/gerenciamento/usuarios" })} className="rounded-md border border-border px-4 py-2 text-sm">Cancelar</button>
          <button disabled={saving} className="rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50">{saving ? "Criando..." : "Criar Usuário"}</button>
        </div>
      </form>
    </main>
  );
