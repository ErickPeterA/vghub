...
AS $function$
BEGIN
  RETURN public.can_manage_dc_row(_user_id, _project_id, _created_by, _etapa, _departamento);
END;
$function$;
...
TO authenticated
USING (
  public.can_manage_dc_row(auth.uid(), project_id, created_by, etapa, departamento)
)
WITH CHECK (
  public.can_manage_dc_row(auth.uid(), project_id, created_by, etapa, departamento)
);
