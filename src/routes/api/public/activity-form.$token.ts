    ...
    handlers: {
      GET: async ({ params }) => {
        ...
        const { data: link, error } = await supabaseAdmin
          .from("activity_links")
          .select(
            "id,project_id,status,expires_at,label,config_id,answered_at,header_answers,draft_header_answers,draft_question_answers,draft_saved_at",
          )
          .eq("token", token)
          .maybeSingle();
        ...
        if (link.status === "cancelled") {
          return Response.json(
            { error: "cancelled", message: "Este link foi cancelado pelo responsável." },
            { status: 410 },
          );
        }
        ...
        if (!config || !config.is_active) {
          return Response.json(
            { error: "inactive", message: "O formulário está inativo no momento." },
            { status: 410 },
          );
        }

        const header = Array.isArray(config.header_schema)
          ? normalizeActivityFields(config.header_schema as ActivityField[]).filter(
              (field) => field?.active ?? true,
            )
          : config.header_schema;
        const questions = Array.isArray(config.questions_schema)
          ? normalizeActivityFields(config.questions_schema as ActivityField[]).filter(
              (field) => field?.active ?? true,
            )
          : config.questions_schema;
